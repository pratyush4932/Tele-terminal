...
function initSharedShell() {
    ...
    sharedShell.onData((data) => {
        if (io) io.to('authenticated').emit('output', data);
    });
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('input', (data) => {
            if (authenticated && sharedShell) {
                ...
                const cmdLower = socket.cmdBuffer.toLowerCase();
                    ...
                    if (appConfig.security) {
                        if (appConfig.security.allowed_cmds) {
                            ...
                            if (allowlist.length > 0) {
                                ...
                                for (const allowed of allowlist) {
                                    if (allowed.startsWith(socket.cmdBuffer) || socket.cmdBuffer.startsWith(allowed)) {
                                        isAllowed = true;
                                        break;
                                    }
                                }
                        ...
                        if (appConfig.security.restricted_cmds) {
                            ...
                            for (const blocked of blocklist) {
                                if (socket.cmdBuffer.includes(blocked)) {
                                    sharedShell.write('\x03'); 
                                    socket.cmdBuffer = '';
